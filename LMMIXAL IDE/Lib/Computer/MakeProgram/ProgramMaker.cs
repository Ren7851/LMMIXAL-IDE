    ...
    public class ProgramMaker
    {
        ...
        public static Program Make(string text, Computer computer)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                //record = Macros.GetInstance().Substitute(record);
                ...
                if(new CSEV("", "", "").Type() == name)
                {
                    list.Add(new CSEV(arg1, arg2, arg3));
                }
                if (new CSN("", "", "").Type() == name)
                {
                    list.Add(new CSN(arg1, arg2, arg3));
                }
                if (new CSNN("", "", "").Type() == name)
                {
                    list.Add(new CSNN(arg1, arg2, arg3));
                }
                if (new CSNP("", "", "").Type() == name)
                {
                    list.Add(new CSNP(arg1, arg2, arg3));
                }
                if (new CSNZ("", "", "").Type() == name)
                {
                    list.Add(new CSNZ(arg1, arg2, arg3));
                }
                if (new CSOD("", "", "").Type() == name)
                {
                    list.Add(new CSOD(arg1, arg2, arg3));
                }
                if (new CSP("", "", "").Type() == name)
                {
                    list.Add(new CSP(arg1, arg2, arg3));
                }
                if (new CSZ("", "", "").Type() == name)
                {
                    list.Add(new CSZ(arg1, arg2, arg3));
                }

                if (new ZSEV("", "", "").Type() == name)
                {
                    list.Add(new ZSEV(arg1, arg2, arg3));
                }
                if (new ZSN("", "", "").Type() == name)
                {
                    list.Add(new ZSN(arg1, arg2, arg3));
                }
                if (new ZSNN("", "", "").Type() == name)
                {
                    list.Add(new ZSNN(arg1, arg2, arg3));
                }
                if (new ZSNP("", "", "").Type() == name)
                {
                    list.Add(new ZSNP(arg1, arg2, arg3));
                }
                if (new ZSNZ("", "", "").Type() == name)
                {
                    list.Add(new ZSNZ(arg1, arg2, arg3));
                }
                if (new ZSOD("", "", "").Type() == name)
                {
                    list.Add(new ZSOD(arg1, arg2, arg3));
                }
                if (new ZSP("", "", "").Type() == name)
                {
                    list.Add(new ZSP(arg1, arg2, arg3));
                }
                if (new ZSZ("", "", "").Type() == name)
                {
                    list.Add(new ZSZ(arg1, arg2, arg3));
                }
                ...
                if ((new Lib.Computer.Commands.OctoBoolCommands.MOR("", "", "").Type() == name))
                {
                    list.Add(new Lib.Computer.Commands.OctoBoolCommands.MOR(arg1, arg2, arg3));
                }

                if ((new Lib.Computer.Commands.OctoBoolCommands.MXOR("", "", "").Type() == name))
                {
                    list.Add(new Lib.Computer.Commands.OctoBoolCommands.MXOR(arg1, arg2, arg3));
                }



                if ((new Lib.Computer.Commands.OctoBoolCommands.CMP("", "", "").Type() == name))
                {
                    list.Add(new Lib.Computer.Commands.OctoBoolCommands.CMP(arg1, arg2, arg3));
                }

                if ((new Lib.Computer.Commands.DoubleArithmeticCommands.DDIF("", "").Type() == name))
                {
                    list.Add(new Lib.Computer.Commands.DoubleArithmeticCommands.DDIF(arg1, arg2));
                }

                if ((new Lib.Computer.Commands.LongArithmeticCommands.LDIF("", "").Type() == name))
                {
                    list.Add(new Lib.Computer.Commands.LongArithmeticCommands.LDIF(arg1, arg2));
                }

                if ((new Lib.Computer.Commands.ByteArithmeticCommands.BDIF("", "").Type() == name))
                {
                    list.Add(new Lib.Computer.Commands.ByteArithmeticCommands.BDIF(arg1, arg2));
                }


                if ((new Lib.Computer.Commands.GOTO.GO("").Type() == name))
                {
                    list.Add(new Lib.Computer.Commands.GOTO.GO(arg1));
                }

                if ((new Lib.Computer.Commands.GOTO.JUMP("").Type() == name))
                {
                    list.Add(new Lib.Computer.Commands.GOTO.JUMP(arg1));
                }
                ...
                if (newSize == lastSize)
                {
                    if (text.Split('\n')[i].TrimStart().Length > 1)
                    {
                        if (text.Split('\n')[i].TrimStart().Substring(0, 2) != "//")
                        {
                            list.Add(new Empty());
                            errors.Add(new KeyValuePair<string, int>(count + ") Unexpected command on the line " + i + ": \"" + text.Split('\n')[i] + "\"", i));
                            count++;
                        }
                    }
                    else 
                ...
                {
                    errors.Add(new KeyValuePair<string,int>(count + ") What the hell on the line " + i + ": \"" + text.Split('\n')[i] + "\"?", i));
                    count++;
                }
            }
            ...
            for (int i = 0; i < list.Count; i++)
            {
                for (int j = 0; j < list[i].Arity; j++)
                {
                    if (list[i].Args[j] == "")
                    {
                        errors.Add(new KeyValuePair<string,int>(count + ") Invalid number of arguments on line " + i + " \"" + text.Split('\n')[i] + "\"", i));
                        count++;
                    }
                }
            }
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (balance < 0)
                {
                    errors.Add(new KeyValuePair<string,int>(count + ") Number of ENDIFs more then number of IFs on the line " + i + " \"" + text.Split('\n')[i] + "\"", i));
                    count++;
                    break;
                }
            }
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (command.Type() == new GOTO("").Type() || command.Type() == new GOTOPOS("", "").Type() || command.Type() == new GOTONEG("", "").Type() || command.Type() == new GOTOZERO("", "").Type())
                {
                    ...
                    if (!isFound)
                    {
                        errors.Add(new KeyValuePair<string,int>(count + ") There is no label for command on the line " + i + " \"" + text.Split('\n')[i] + "\"", i));
                        count++;
                    }
                }
                ...
                if (command.Type() == new IF("").Type())
                {
                   ...
                   {
                      errors.Add(new KeyValuePair<string,int>(count + ") Cannot parse " + command.Args[0] + " on the line "+i+" \"" + text.Split('\n')[i] + "\" Have you forgotten <> or {}?", i));
                      count++;
                   }
                }
            ...
            for (int i = 0; i < list.Count; i++)
            {
                for (int j = 0; j < list[i].OctoArgs.Count; j++)
                {
                    ...
                    {
                        errors.Add(new KeyValuePair<string,int>(count + ") Unexpected octo value " + list[i].Args[list[i].OctoArgs[j]] + " on the line " + i + " \"" + text.Split('\n')[i] + "\"", i));
                        count++;
                    }
                }
                ...
                for (int j = 0; j < list[i].ByteArgs.Count; j++)
                {
                    ...
                    {
                        errors.Add(new KeyValuePair<string,int>(count + ") Unexpected byte value " + list[i].Args[list[i].ByteArgs[j]] + " on the line " + i + " \"" + text.Split('\n')[i] + "\"", i));
                        count++;
                    }
                }
                ...
                for (int j = 0; j < list[i].HexArgs.Count; j++)
                {
                    ...
                    {
                        errors.Add(new KeyValuePair<string,int>(count + ") Invalid hex value " + arg + " on the line " + i + " \"" + text.Split('\n')[i] + "\"", i));
                        count++;
                    }
                }
            }
