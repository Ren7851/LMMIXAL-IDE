...
namespace Lib.Utils
{
    public class Utils
    {
        ...
        public static OctoByte ParseAddress(string address, Lib.Computer.Computer computer)
        {
            ...
            if(address == "PEEK")
            {
                return new OctoByte(computer.Peek());
            }
            ...
            if (address.IndexOf("$") != -1)
            {
                computer.selectAddress(true, HexToInt(address.Substring(1)));
                return computer.memory.GetRegister(HexToInt(address.Substring(1)));
            }
            else
            {
                int hti = HexToInt(address);
                computer.selectAddress(false, hti);
